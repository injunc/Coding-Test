...
class Solution {
    public int solution(int a, int b, int c, int d) {
        ...
        for(int q : Arrays.asList(a,b,c,d)){
        ...
        // 1:1:1:1 이면
        if(size == 4) return Arrays.asList(a,b,c,d).stream().mapToInt(i->i).min().getAsInt();
        ...
        // 2:2 이면
        if(map.get(a) == 2){
            int[] sum = Arrays.asList(a,b,c,d).stream().distinct().mapToInt(i->i).toArray();
